// Copyright (c) Meta Platforms, Inc. and affiliates.
// All rights reserved.
//
// This source code is licensed under the terms described in the LICENSE file in
// the root directory of this source tree.

import type { DeleteLogsAfterMutation } from "./__generated__/DeleteLogsAfterMutation.graphql";

import { commitMutation, Environment } from "react-relay";
import { graphql } from "relay-runtime";
import { Notify } from "../contexts/NotificationsContextProvider";
import { GqlError } from "../utils/errors";

const DeleteLogsAfterMutation = graphql`
  mutation DeleteLogsAfterMutation(
    $sessionId: String!
    $id: String!
    $edit: String
  ) {
    deleteLogsAfter(sessionId: $sessionId, id: $id, edit: $edit) {
      scenarioId
    }
  }
`;

export function deleteLogsAfterMutation(
  environment: Environment,
  sessionId: string,
  notify: Notify,
  id: string,
  {
    edit,
  }: {
    edit?: string;
  },
) {
  const variables = {
    sessionId,
    id,
    edit,
  };
  commitMutation<DeleteLogsAfterMutation>(environment, {
    mutation: DeleteLogsAfterMutation,
    variables,
    onCompleted: (response, errors) => {
      if (errors) {
        console.log("Errors:", errors);
        notify({ message: "Errors: " + JSON.stringify(errors), type: "error" });
      } else {
        console.log("Response:", response);
      }
    },
    onError: (err: GqlError) => {
      console.error(err);
      notify({
        message: err?.messageFormat ?? "Error: " + JSON.stringify(err),
        type: "error",
      });
    },
  });
}
